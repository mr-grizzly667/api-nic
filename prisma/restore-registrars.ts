...
async function main() {
  ...
  for (const dbReg of dbRegistrars) {
    // Find matching mock
    const match = MOCK_REGISTRARS.find(m => 
      m.name.toLowerCase() === dbReg.name.toLowerCase() || 
    ...
    if (match) {
      await prisma.registrar.update({
        ...
        data: {
          ...
          logo: '' // Reset logo so the frontend uses the initials badge
        }
      });
      restored++;
      console.log(`Restored: ${dbReg.name} (Domains: ${match.domainsCount}, URL: ${match.website})`);
    } else {
      // Fallback if no exact match (shouldn't happen with the current list)
